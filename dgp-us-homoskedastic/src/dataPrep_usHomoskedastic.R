## Uniform Spatial Homoskedastic DiD Data Preparation for JAGS ##
...
data <- read.table("../data/simData_usHomoskedastic.txt", header = T)
# data

X <- data %>% dplyr::select(-c(region, y))
# design matrix

W <- read.table("../data/adjMatrix_usHomoskedastic.txt") %>%
  as.matrix()
# adjacency matrix
...
lt.Data.uspatial <- list(y = data$y, trt = X$treatment, time = X$time, car.mu = car.mu, 
...
lt.Data.cls <- list(trt = X$treatment, time = X$time, y = data$y,
                     q = q, N = N)
# classical model
